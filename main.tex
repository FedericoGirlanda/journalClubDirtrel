...
\item Quadratic cost of the form $\delta x^T_i Q^\ell \delta x_i + \delta u^T_i R^\ell \delta u_i$, where $Q^\ell \geq 0$ and $R^\ell \geq 0$ are positive semidefinite cost matrices.
...
\[\ell_W(x_{1:N}, u_{1:N-1}) \approx \frac{1}{Vol(W)}\int_{W} (\delta x^T_N Q^\ell_N \delta x_N + \sum_{i=1}^{N-1} (\delta x^T_i Q^\ell \delta x_i + \delta u^T_i R^\ell \delta u_i) )\,dW\] \\
...
\[\ell_W(x_{1:N}, u_{1:N-1}) = Tr(Q_N^\ell E_N) + \sum_{i=1}^{N-1} Tr((Q^\ell + K^T_i R^\ell K_i)E_i)\]
\end{frame}

\begin{frame}{The DIRTREL Algorithm}
In addition to augmenting the DIRTRAN optimization problem with $\ell_W (x_{1:N}, u_{1:N-1})$, we must also ensure that the closed-loop system obeys state and input constraints.
\vspace{0.5 cm}
...
\vspace{0.5 cm}
\begin{figure}
\includegraphics[width=0.8\linewidth, height=0.7\linewidth]{figures/trajCompManchester.png}
\caption{Figure 5: Direct Transcription vs DIRTREL nominal trajectories from the reference paper}
\end{figure}
\end{minipage}
\hspace{0.5 cm}
\begin{minipage}{.45\linewidth}
\centering
\begin{figure}
\includegraphics[width=0.9\linewidth, height=0.8\linewidth]{figures/DIRTRELvsDIRTRAN.pdf}
\caption{Figure 6: Direct Transcription vs DIRTREL nominal trajectories from my implementation}
\end{figure}
\end{minipage}
\end{frame}
...
\begin{figure}
\includegraphics[width=0.8\linewidth, height=0.3\linewidth]{figures/invariantFunnel.png}
\caption{Figure A1: Conceptual depiction of an invariant funnel around a nominal trajectory}
\end{figure}
\end{frame}

\begin{frame}{Connections to Sum-of-Squares Methods}
By defining \\
\[S(w) = w^T D^{-1} w - 1 \leq 0 \quad and \quad V(t, \delta x) = \delta x^T E(t) \delta x - 1 \leq 0 \]
we can define a robust invariant funnel by imposing \\
\[\dot{V}(t, \delta x, w) \leq 0\]
when $V(t, \delta x) = 0$ and for all disturbances $w \in W = \left\{w | S(w) \leq 0\right\}$
\vspace{0.5 cm}
\begin{block}{}
It can be shown that imposing the SOS formulation with this notation will lead to obtain the propagation rule for E that is used to compute the robust cost in DIRTREL.
\end{block}
\end{frame}
